...
def get_tw_listed_stocks():
    try:
        ...
        for item in df['有價證券代號及名稱']:
            code = str(item).split(full_space)[0].strip()
...
def scan_logic(symbol, params):
    try:
        df = yf.download(symbol, period="150d", interval="1d", progress=False, threads=False, timeout=10)
        ...
        if avg_bw < (params['bw_limit']/100) and price_break and vol_ok and open_ok and macd_ok:
            return {
                ...
                "壓縮帶寬": f"{round(avg_bw*100, 2)}%", "df": df
            }
    except:
        pass
    return None
...
st.title("🛡️ 台股「長週期橫盤突破」量化篩選 (含一鍵匯出)")
...
with st.sidebar:
    ...
    st.divider()
    stock_limit = st.number_input("掃描數量", 10, 1500, 1500)
...
if st.button("🔍 開始全市場大掃描"):
    ...
    if hits:
        ...
        st.download_button(label="📥 下載本次篩選清單 (CSV)", data=csv, file_name=f"stock_scan_{datetime.now().strftime('%Y%m%d')}.csv", mime='text/csv')

        for hit in hits:
            with st.expander(f"💎 {hit['代號']} | 價: {hit['現價']} | 壓縮: {hit['壓縮帶寬']}"):
                ...
                fig.update_layout(xaxis_rangeslider_visible=False, height=550, hovermode="x unified")
                st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("查無標的，請放寬參數再試。")
